...
def get_X_ranked_df(toi_path, tic_path, kp_file=r'data/known_planets/known_planets.csv', include_qlp=False, num_to_rank=5, dec_cut=-20, k_amp_cut=2):
    ...
    # Known planets
    df = tess.append(kps[np.logical_and(kps['K_amp'] > k_amp_cut, kps['TSM'] > 10)],sort=False)
    ...
    useful_cols = ['Source Pipeline', 'Full TOI ID', rp_key, pp_key, Ts_key, Fp_key,
                   'Planet Equilibrium Temperature (K) Value', 'mass_flag', 'V mag','K_amp', 'TSM', 't_HIRES', 'X', 'TIC']
    # useful_cols = ['Source Pipeline', 'Full TOI ID', 'Planet Radius Value',
    #    'Orbital Period Value', 'Effective Temperature Value',
    #    'Effective Stellar Flux Value',
    #    'Planet Equilibrium Temperature (K) Value', 'mass_flag', 'V mag',
    #    'K_amp', 'TSM', 't_HIRES', 'X', 'priority']
...
def get_target_list(save_fname=None, toi_folder='data/toi/', tic_folder='data/exofop/', selected_TOIs_folder='data/TKS/', include_qlp=False, verbose=True, num_to_rank=5, k_amp_cut=2., min_TSM=0.):
    ...
    ## NOTE: Uncomment the line above to merge on selected_TOIs! - Joey, 2020/08/04
    X_tois_df = X_df.copy() # Not merging on selected TOIs for the moment!!!
...
def summary_plot(sc3_df, pick_tois_df, benchmark_targets=None, id_key='Full TOI ID', hist_bin_num=10):
    ...
    for ax, x_axis_key in zip([ax_vmag, ax_kamp, ax_rad], ['V mag', 'K_amp', 'Planet Radius Value']):
        for index, row in sc3_df.iterrows():
            ...
            ax.plot(row[x_axis_key], row['X'], '*', alpha=0.7, color=color, markersize=5)
            ...
            #              ha='left',
            #              clip_on=True)
        ax.plot(row[x_axis_key], row['X'], '*', alpha=0.7, label='Picked by SC3', markersize=5) # Plot the last value again just to get the label
