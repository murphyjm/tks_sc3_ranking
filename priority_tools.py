...
def binning_function(dataset,bins,id_key='Full TOI ID', sort_val='TSM'):
    ...
    pre_bin = dataset.assign(
        radius_bin = pd.cut(dataset[rp_key],bins=rad_bins,labels = np.arange(len(rad_bins) - 1) + 1),
        insol_bin = pd.cut(dataset[Fp_key],bins=fpl_bins,labels = np.arange(len(fpl_bins) - 1) + 1),
        st_Teff_bin = pd.cut(dataset[Ts_key],bins=tef_bins,labels = np.arange(len(tef_bins) - 1) + 1)
    )
